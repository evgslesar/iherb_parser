...
async def get_items_json(session, link):
    items_json = []
    ...
    async with session.get(url=url, data=payload, headers=headers) as response:
        response_jsn = await response.json()

        items_json.append(response_jsn)
        # print(items_json)
...
def save_to_db(items_data):
    ...
    # conn = sqlite3.connect('iherb_data.db')
    # df.to_sql("async_iherb_data_info", conn, index=False, if_exists='replace')
    # conn.close()
    df.to_csv("async_iherb_data.csv")

async def main(links):
    async with aiohttp.ClientSession() as session:

        tasks = (get_items_json(session, link) for link in links)
...
if __name__ == '__main__':
    ...
    links = get_pages()
    items_json = [item[0] for item in asyncio.run(main(links))]
